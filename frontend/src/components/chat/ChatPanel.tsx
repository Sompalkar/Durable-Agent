...
export function ChatPanel({
  ...
  /** Model and effort, shown on the composer's own toolbar row. */
  ...
  return (
    <section className="flex min-h-0 min-w-0 flex-1 flex-col bg-canvas">
      <div className="flex-1 overflow-y-auto overscroll-contain">
        <div className="mx-auto max-w-3xl space-y-5 px-3 py-6 sm:px-4 sm:py-8">
          {isEmpty ? (
            <EmptyState
                ...
                <div className="grid gap-2 pt-2 sm:grid-cols-3">
                  {SUGGESTIONS.map((suggestion) => (
                    <button
                      ...
                      className="rounded-xl border border-line bg-panel px-3.5 py-3 text-left text-[13px] leading-relaxed text-ink-soft shadow-soft transition-colors hover:border-line-strong hover:bg-hover hover:text-ink"
                    >
                      {suggestion}
                    </button>
                  ))}
